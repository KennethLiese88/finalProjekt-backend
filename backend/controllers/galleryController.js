...
export async function uploadImage(req, res) {
  try {
    ...
    res.status(201).json({ msg: "Image successfully uploaded!", imageUrl });
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: "Error during upload!" });
  }
}
...
export async function allImages(req, res) {
  try {
    ...
    console.error(error);
    res.status(500).json({ msg: "Error retrieving the images!" });
  }
}
